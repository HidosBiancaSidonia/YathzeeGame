...
LowerScores::LowerScores(sf::RenderWindow& window)
{
	...
	if (!background.loadFromFile("images\\LowerScored.jpg"))
	{
		std::cout << "Error: Could not display image" << std::endl;
		Logger logger(logFile, Logger::Level::Error);
		logger.Log("Could not display Lower Score image!", Logger::Level::Error);
	}
    ...
    backButtonImage.setPosition(890.0f, 610.0f);
