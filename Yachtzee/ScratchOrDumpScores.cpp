...
ScratchOrDumpScores::ScratchOrDumpScores(sf::RenderWindow& window)
{
	...
	if (!background.loadFromFile("images\\Scratch.jpg"))
	{
		std::cout << "Error: Could not display image" << std::endl;
		Logger logger(logFile, Logger::Level::Error);
		logger.Log("Could not display Scratch or Dump image!", Logger::Level::Error);
	}
